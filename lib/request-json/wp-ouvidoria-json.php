		...
		if(!empty($_POST['nome']) && !empty($_POST['email']) && !empty($_POST['assunto']) && !empty($_POST['telefone']) && !empty($_POST['mensagem'])){
			if(Util::ValidaEmail($_POST['email'])){
				//if (isset($_SESSION['simpleCaptchaAnswer']) && $_POST['captchaSelection'] == $_SESSION['simpleCaptchaAnswer']){
					...
					$nome = $_POST['nome'];
					$assunto = (!empty($_POST['assunto']))? $_POST['assunto'] : get_option('assunto_ouvidoria');
					$email = $_POST['email'];
					$estado = $_POST['estado'];
					$telefone = $_POST['telefone'];
					$ja_cliente = $_POST['ja_cliente'];
					...
					//$mail->From = get_option('email_receptor_ouvidoria');
					...
					$mail->AddAddress(get_option('email_receptor_ouvidoria'),"$nome");
					...
					if($anexado){
						if(!$mail->Send()){
							$value = array('sucesso' => false,'msg' => get_option('msg_erro_ouvidoria').' '.$mail->ErrorInfo, 'erro' => $mail->ErrorInfo);
							@unlink($pasta.$nome_atual);
						}else{
							$value = array('msg' => get_option('msg_envio_ouvidoria'), 'sucesso' => 'true');
						}
					}else{
						$value = array('sucesso' => false,'msg' => "Não foi possivel enviar o e-mail com anexo");
					}
					Util::CadastroFormulario(array('nome' => $nome, 'email' => $email, 'assunto' => $assunto, 'estado' => $estado, 'telefone' => $telefone, 'ja_cliente' => $ja_cliente, 'mensagem' => $mensagem),'ouvidoria');
				/*} else {
			...
			$value = array('msg' => get_option('msg_preencimento_ouvidoria'));
		}
